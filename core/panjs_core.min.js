...
(function(){panjs.logger=null;panjs.root={id:"root"};panjs.loader=null;panjs.messages={CLASSNAME_MATCHS_FILENAME:"The name of the class (%1) must match the file name %2 (case sensitive)",LESS_IE8:"less.js is not fully compatible with IE%1 : transform less code in css",LESS_NOT_LOADED:"LESS not loaded. Use panjs_core_with_less.min.js or load LESS (before panJs)"};panjs.getMessage=function(a){for(var c=1;c<arguments.length;c++){var d=arguments[c];null==d&&(d="null");a=a.replace("%"+c,d)}return a};window.less=
"undefined"!=typeof panjs.less?panjs.less:{env:"production",async:!0,fileAsync:!1,poll:1E3,functions:{},dumpLineNumbers:"comments",relativeUrls:!1,rootpath:":/a.com/"};for(var a=0;a<panjs.namespaces.length;a++)"undefined"!=typeof panjs.namespaces[a]&&(panjs.namespaces[a].path=panjs.namespaces[a].path.replace("{version}",panjs.version));panjs.createComponent=function(a,c){var d=panjs.getClassNameFromClassPath(a);if("undefined"==typeof window[d]){var e=panjs.loader.usesComponent(a);if(!e.result){uses("core.display.TerrorElement.html");
var f=new TerrorElement(e);return f}}try{f=new window[d](c)}catch(g){logger.error("Error while instantiating "+d+": "+g),uses("core.display.TerrorElement.html"),f=new TerrorElement({message:g,path:e.path,url:e.url,className:d})}f._onInitialized&&f._onInitialized();return f};panjs.load=function(a){panjs._load(a)};panjs._load=function(a){panjs.iever=getIEVersion();panjs.chromever=getChromeVersion();panjs.ffver=getFirefoxVersion();0==arguments.length&&(a=$(document.body));var c=[];null!=a.attr("data-compo")?
