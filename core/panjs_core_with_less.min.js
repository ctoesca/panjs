...
(function(){panjs.logger=null;panjs.root={id:"root"};panjs.loader=null;panjs.iever=getIEVersion();panjs.chromever=getChromeVersion();panjs.ffver=getFirefoxVersion();panjs.messages={CLASSNAME_MATCHS_FILENAME:"The name of the class (%1) must match the file name %2 (case sensitive)",LESS_IE8:"less.js is not fully compatible with IE%1 : transform less code in css",LESS_NOT_LOADED:"LESS not loaded. Use panjs_core_with_less.min.js or load LESS (before panJs)"};panjs.getMessage=function(a){for(var c=1;c<
arguments.length;c++){var d=arguments[c];null==d&&(d="null");a=a.replace("%"+c,d)}return a};window.less="undefined"!=typeof panjs.less?panjs.less:{env:"production",async:!0,fileAsync:!1,poll:1E3,functions:{},dumpLineNumbers:"comments",relativeUrls:!1,rootpath:":/a.com/"};for(var a=0;a<panjs.namespaces.length;a++)"undefined"!=typeof panjs.namespaces[a]&&(panjs.namespaces[a].path=panjs.namespaces[a].path.replace("{version}",panjs.version));panjs.createComponent=function(a,c){var d=panjs.getClassNameFromClassPath(a);
if("undefined"==typeof window[d]){var e=panjs.loader.usesComponent(a);if(!e.result){uses("core.display.TerrorElement.html");var f=new TerrorElement(e);return f}}try{f=new window[d](c)}catch(g){logger.error("Error while instantiating "+d+": "+g),uses("core.display.TerrorElement.html"),f=new TerrorElement({message:g,path:e.path,url:e.url,className:d})}f._onInitialized&&f._onInitialized();return f};panjs.load=function(a){panjs._load(a)};panjs._load=function(a){0==arguments.length&&(a=$(document.body));
var c=[];null!=a.attr("data-compo")?c.push(a):c=a.getElements("[data-compo]");for(var d=0;d<c.length;d++){var e=c[d],f=e.attr("data-compo"),g="false"!==e.attr("autoload"),h=e.attr("id");!0==g?(f=panjs.createComponent(f,{elem:e[0]}),e.replaceWith(f.container),f.container[0].owner=f):(uses("core.display.TproxyDisplayObject"),f=new TproxyDisplayObject({sourceElement:e[0]}),e.hide(),e[0].setAttribute("id",panjs.root.id+"_"+h),e[0].originalId=h,e[0].owner=panjs.root);panjs.root[h]=f}logger.info("READY");
...
/*##############################################
###########  D:\github\panjs\panjs\lib\less\1.4.2\less.min.js#################
##############################################*/
