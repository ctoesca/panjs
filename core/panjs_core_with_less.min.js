...
this.hash=window.location.hash.droite("#");this.keysTmp=this.decode(this.hash);for(k in this.keys)logger.debug("keys."+k+" = "+this.keys[k]+", hash."+k+" = "+this.keysTmp[k]),this.keysTmp[k]!=this.keys[k]&&(this.keys[k]=this.keysTmp[k]||null,"undefined"!=typeof this.listeners[k]&&(logger.debug("Le composant "+k+" va \u00eatre notifi\u00e9 que son hash a chang\u00e9: hash="+this.keys[k]),this._dispatchOnhashchange(k,this.keys[k])));a=new Tevent(Trouter.HASH_CHANGE,this.keys);this.dispatchEvent(a);
