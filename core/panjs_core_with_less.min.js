...
(function(){panjs.logger=null;panjs.root={id:"root"};panjs.loader=null;panjs.iever=getIEVersion();panjs.chromever=getChromeVersion();panjs.ffver=getFirefoxVersion();panjs.stack=[];panjs.stackLevel=0;panjs.messages={CLASSNAME_MATCHS_FILENAME:"The name of the class (%1) must match the file name %2 (case sensitive)",LESS_IE8:"less is not fully compatible with IE%1 : transform less code in css",LESS_NOT_LOADED:"LESS not loaded. Use panjs_core_with_less.min.js or load LESS (before panJs)"};panjs.getMessage=
...
")",className:b,classPath:a,url:c,path:"?"};if("undefined"==typeof window[b]){logger.groupStart();0==this._count&&logger.info(" ************ "," USESCOMPONENT ",a," ************ ");this._count++;var d=panjs.getAbsoluteUrlFromClassPath(a),c=this.getUrlWithVersion(d),e=this.loadFile(c);if(e.result){try{var f=getXmlDocument(e.data)}catch(g){return logger.error(g),{result:!1,message:g,className:b,classPath:a,url:c,path:d}}for(var e=f.getElementsByTagName("head")[0],f=f.getElementsByTagName("body")[0],
h=[],m=[],q=d.substring(0,d.lastIndexOf("/")),n=0;n<e.childNodes.length;n++){var r=e.childNodes[n],s=r.nodeName.toLowerCase();if("script"==s)try{this.addScriptNode(r,q,b)}catch(t){var l="Error processing <"+s+"> in "+b+" => "+t;logger.error(d,l);return{result:!1,message:l,className:b,classPath:a,url:c,path:d,stack:t.stack}}else"link"==s?h.push(r):"style"==s&&m.push(r)}if("undefined"==typeof window[b])return l=a+" class has not been loaded. ",panjs.lastDefinedClassName!=b&&(l+="\n"+panjs.getMessage(panjs.messages.CLASSNAME_MATCHS_FILENAME,
panjs.lastDefinedClassName,d)),logger.error(l),{result:!1,message:l,className:b,classPath:a,url:c,path:d,stack:null};for(n=0;n<h.length;n++)this.addLinkNode(h[n],q);for(n=0;n<m.length;n++)logger.debug("Load style ",a),this.addStyleNode(m[n]);if(defined(f))try{var l=getXml(f),u=window[b].prototype.parentClassName;defined(window[u].prototype.html)&&(l=window[u].prototype.html.replace("\x3c!--CONTENT--\x3e",l));l=l.replace(/<body>/gi,"").replace(/<\/body>/gi,"");l=l.replace(/\\r/gi,"").replace(/\\n/gi,
"").trim();window[b].prototype.html=l;if(8==panjs.iever){var w=$("<div>"+l+"</div>");window[b].prototype.html=w[0].innerHTML}window[b].prototype.bodyNode=f}catch(x){return{result:!1,message:x.message,className:b,classPath:a,url:c,path:d}}window[b].prototype.classPath=a;window[b].prototype.classPathDir=this.getClassPathDir(a);window[b].prototype.className=b;window[b].prototype.dirPath=q;window[b].lastId=0;this._count--;0==this._count&&logger.debug(" ************ "," END USESCOMPONENT ",a," ************ ");
...
!0!=e&&(e=0<=d.includeIn.indexOf(h)),null!=d.excludeFrom&&!0!=f&&(f=0<=d.excludeFrom.indexOf(h))))break}b=!0;if(null!=e||null!=f)logger.debug("setState "+this.currentState+": affichage "+d.id+", included="+e+", excluded="+f),this._setStateElementVisible((!0==e||null==e)&&(!1==f||null==f),d)}for(g=0;g<this.currentState.length;g++)this.dispatchEvent(new Tevent(Tevent.STATE_CHANGED,{oldStates:a,state:this.currentState[g]}))},_populateElements:function(a,b,c){for(var d=0;d<a.childNodes.length;d++){var e=
a.childNodes[d];if(1==e.nodeType){var f=e.getAttribute("id"),g=e.getAttribute("data-compo");null==g?(b&&null!=f&&(defined(this[f])&&logger.error('1-Duplication du id "'+f+'" sur objet '+this.className),this[f]=$(e),c.push(f)),this._populateElements(e,b,c)):g==this.classPath?logger.error("R\u00e9f\u00e9rence circulaire dans "+this.classPath):"false"!==e.getAttribute("autoload")?(g=panjs.createComponent(g,{elem:e,parent:this}),g.parent=this,b&&null!=f&&(defined(this[f])&&logger.error('2-Duplication du id "',
f,'" sur objet ',this.className),this[f]=g,c.push(f)),!0==g.visible?($(e).replaceWith(g.container),g.container[0].owner=g):logger.info("PAS DE replaceWith pour ",g.id," visible = ",g.visible," ",typeof g.visible),e.loaded=!0,g.loaded=!0):(e.setAttribute("id",this.id+"_"+f),uses("core.display.TproxyDisplayObject"),g=new TproxyDisplayObject({sourceElement:e}),g.parent=this,e.loaded=!1,$(e).hide(),b&&null!=f&&(this[f]=g));e.originalId=f;e.owner=this;this.processStates(e)}else 8==e.nodeType&&(f="",f=
-1==panjs.iever||8<panjs.iever?e.textContent:e.text.droite("\x3c!--").gauche("--\x3e").trim(),f=f.toUpperCase(),"CONTENT"==f&&(f=document.createElement("div"),f.setAttribute("data-class","content"),a.replaceChild(f,e),this.content=$(f)))}return c}});TdisplayObjectContainer._listeIdElements=[];
...
return TrestClient.lastTokenId},_getSuccessEvent:function(a,b,c){logger.debug("X-Time="+b.getResponseHeader("X-Time"));a=new Tevent(Tevent.SUCCESS,a);a.xTime=b.getResponseHeader("X-Time");return a},_getErrorEvent:function(a,b,c,d,e){b={method:b,responseText:"",statusText:"Erreur inconnue",status:0,url:"url=?"};defined(a,"statusText")?b.statusText=a.statusText:b.statusText="";defined(c)&&(b.statusText=c);defined(d)&&(b.url=d);defined(a,"responseText")&&(b.responseText=a.responseText);""==b.responseText&&
(b.responseText=b.statusText);defined(a,"status")&&(b.status=a.status);0==b.status&&(b.status=1);b.token=e;try{b.data=JSON.parse(b.responseText)}catch(f){b.data=b.responseText}return new Tevent(Tevent.ERROR,b)},post:function(a,b,c,d,e,f){this._call("POST",a,b,c,d||null,e||null,f)},get:function(a,b,c,d,e,f){this._call("GET",a,b,c,d||null,e||null,f)},getText:function(a,b,c,d,e,f){this._call("GET",a,b,c,d||null,e||null,f,"text")},put:function(a,b,c,d,e,f){this._call("PUT",a,b,c,d||null,e||null,f)},del:function(a,
b,c,d,e,f){this._call("DELETE",a,b,c,d||null,e||null,f)},_call:function(a,b,c,d,e,f,g,h){g.id=this.getNewTokenId();var m="";if(defined(c))if("object"==typeof c){m="?";for(p in c)m=m+p+"="+c[p]+"&";"?"==m&&(m="")}else m="?"+c;this.extraUrlParams&&(m+="&"+this.extraUrlParams);var q=this.baseUrl+b+m;8>arguments.length&&(h=this.dataType);g.dataType=h;logger.debug("Appel ",a,": ",q,", dataType="+h);return $.ajax({url:q,async:!0,data:d,context:this,type:a,xhrFields:{withCredentials:!0},dataType:h,success:function(c,
d,h){logger.debug("Requ\u00eate AJAX termin\u00e9e avec succ\u00e8s: ",b);if(null!=this.exitCodeFieldName&&(d=c[this.exitCodeFieldName],defined(d)&&200!=d)){c=this._getErrorEvent(h,a,c[this.errorTextFieldName],q,g);this.dispatchEvent(c);defined(f)&&f(c,g);return}c=this._getSuccessEvent(c,h,g);this.dispatchEvent(c);defined(e)&&e(c,g)},error:function(b,c,d){b=this._getErrorEvent(b,a,d,q,g);this.dispatchEvent(b);defined(f)&&f(b,g)}})}});
