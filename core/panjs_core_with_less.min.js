...
0;b<a.length&&(null==a[b]||(c=a[b],null==f.currentTarget&&(f.currentTarget=this),f.bind=c.bind,f.relatedData=c.data,null==f.bind?c.listener(f):c.listener.call(f.bind,f),!f.cancelled));b++);},on:function(a,b,c,d){4>arguments.length&&(d=null);3>arguments.length&&(c=null);var e={listener:b,data:d,bind:c};this.hasEventListener(a)||(this._listeners[a]=[]);for(var f=0;f<this._listeners[a].length;f++)if(null==this._listeners[a][f]){this._listeners[a][f]=e;return}this._listeners[a].push(e)},off:function(a,
...
b);c&&(c.startsWith("{{")?(c=c.droite("{{").gauche("}}"),c=(new Function(c)).bind(this),a.on(b,c)):(new Function('arguments[0].on("'+b+'", '+c+".bind(this))")).bind(this)(a,b))},bindEvents:function(a){for(var b=["click","change","blur","keyup","dblclick"],c=0;c<b.length;c++)this.bindEvent(a,b[c]);this.bind(a)},bind:function(a){var b=a.html();if(b.match(/\{\{.*\}\}/)){var c=doT.template(b).bind(this);this.__bindings.html.push({template:b,element:a,tempFn:c})}},renderBindings:function(){for(var a=0;a<
this.__bindings.html.length;a++){var b=this.__bindings.html[a];try{b.element.html(b.tempFn(this))}catch(c){logger.error(c)}}},free:function(){TdisplayObjectContainer._super.free.call(this)},__OnPropChanged:function(a,b,c,d){logger.debug("__OnPropChanged : propName = "+a+", oldValue = "+b+", newValue = "+c);this.renderBindings()},processStates:function(a){var b=a.getAttribute("includeIn");null!=b&&(b=0<=b.indexOf(",")?b.split(","):[b]);var c=a.getAttribute("excludeFrom");null!=c&&(c=0<=c.indexOf(",")?
