...
/*##############################################
###########  D:\github\panjs\panjs\core\panjs_core.min.js#################
##############################################*/
...
if(!e.result){uses("core.display.TerrorElement.html");var f=new TerrorElement(e);return f}}try{f=new window[d](c)}catch(g){logger.error("Error while instantiating "+d+": "+g),uses("core.display.TerrorElement.html"),f=new TerrorElement({message:g,path:e.path,url:e.url,className:d})}f._onInitialized&&f._onInitialized();return f};panjs.load=function(a){0==arguments.length&&(a=$(document.body));for(var c=a.getElements("[data-compo]"),d=0;d<c.length;d++){var e=c[d],f=e.attr("data-compo"),g="false"!==e.attr("autoload"),
h=e.attr("id");!0==g?(f=panjs.createComponent(f,{elem:e[0]}),e.replaceWith(f.container),f.container[0].owner=f):(uses("core.display.TproxyDisplayObject"),f=new TproxyDisplayObject({sourceElement:e[0]}),e.hide(),e[0].setAttribute("id",panjs.root.id+"_"+h),e[0].originalId=h,e[0].owner=panjs.root);panjs.root[h]=f}logger.info("READY");"undefined"!=typeof onReady&&onReady()};panjs.getClassNameFromClassPath=function(a){var c=".";if(a.startsWith("http://")||a.startsWith("https://")||"/"==a[0])c="/";var d=
a;a.endsWith(".html")&&(d=a.removeEnd(".html"));a=d.split(c);return d=a[a.length-1]};panjs.getAbsoluteUrlFromClassPath=function(a){var c=a.endsWith(".html"),d="";if(a.startsWith("http://")||"/"==a[0])d=c?a:a+".js";else for(var e=0;e<panjs.namespaces.length;e++){var f=panjs.namespaces[e];if(a.startsWith(f.name)){d=a.removeEnd(".html").replace(/\./g,"/");d=d.replace(f.name,f.path);d=c?d+".html":d+".js";break}}return d};panjs.isAbsoluteUrl=function(a){a=a.toLowerCase();return"/"==a[0]||a.toLowerCase().startsWith("http://")||
...
c+")"),exec(b)):(b=panjs.transformUrl(a.getAttribute("src"),b),b=this.getUrlWithVersion(b),this.loadedJs[b.toLowerCase()]||(b=this.loadFile(b),null!=c&&"text/x-class-definition"==a.getAttribute("type")&&(b.data=this.processCode(b.data,c)),exec(b.data)));return!0}});panjs.loader=new Tloader;
...
"?";for(p in c)h=h+p+"="+c[p]+"&";"?"==h&&(h="")}else h="?"+c;this.extraUrlParams&&(h+="&"+this.extraUrlParams);var m=this.baseUrl+b+h;logger.debug("Appel ",a,": ",m,", dataType="+this.dataType);return $.ajax({url:m,async:!0,data:d,context:this,type:a,xhrFields:{withCredentials:!0},dataType:this.dataType,success:function(c,d,h){logger.debug("Requ\u00eate AJAX termin\u00e9e avec succ\u00e8s: ",b);if(null!=this.exitCodeFieldName&&(d=c[this.exitCodeFieldName],defined(d)&&200!=d)){c=this._getErrorEvent(h,
a,c[this.errorTextFieldName],m,g);this.dispatchEvent(c);defined(f)&&f(c,g);return}c=this._getSuccessEvent(c,h,g);this.dispatchEvent(c);defined(e)&&e(c,g)},error:function(b,c,d){b=this._getErrorEvent(b,a,d,m,g);this.dispatchEvent(b);defined(f)&&f(b,g)}})}});


/*##############################################
###########  \\ns382015.ovh.net\nexilearn\production\apache\portails\cdn\libs\less\1.4.2\less.min.js#################
##############################################*/
