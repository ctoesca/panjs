		...
		*/
		...
		var req = $.ajax({ 
				...
				success: function(data, textStatus, req) {
					...
					if (token.aborted){
						logger.debug("Requête AJAX %1 ANNULEE sur %2", token.requestId, url);
						
						var e = this._getErrorEvent(req, method, "aborted", path, token);
						...
						logger.debug("Requête AJAX %1 terminée avec succès sur %2", token.requestId, url);
						if (logger.debug)
				...
				error: function( jqXHR, textStatus, errorThrown) {
					...
					logger.debug("status="+jqXHR.status);
