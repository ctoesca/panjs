...
logger.error("Unable to inherit from "+b+": class is not loaded"),null;try{var m=panjs._classes[b]?panjs._classes[b].Class.extend(c,a,f):window[f].extend(c,a,f);panjs._classes[g]={Class:m,isComponent:e};m.prototype.classPath=g;m.prototype.className=a;window[a]=d?new m:m;for(var q=g.split("."),e=window,f=0;f<q.length;f++){var l=q[f];f==q.length-1?e[l]=window[a]:"undefined"==typeof e[l]&&(e[l]={});e=e[l]}}catch(n){alert(a+" "+n)}__CLASSPATH__=null;panjs._lastDefinedClassName=a}
...
{result:!1,exception:"",status:"erreur inconnue"};logger.debug("Loadfile SYNC %1",a);jQuery.support.cors=!0;var g=jQuery.ajax({url:a,method:"GET",cache:!0,async:!1,dataType:"html",processData:!1,context:this,beforeSend:function(a){},success:function(e,g,l){f={result:!0,data:e};if(b){for(var h in localStorage)h.startsWith(c)&&!h.startsWith(c+"."+panjs.appVersion)&&(logger.debug("Removing old version from locaStorage: %1",h),localStorage.removeItem(h));try{localStorage.setItem(d,e)}catch(p){logger.debug(p+
...
a);logger.groupStart();c.path=panjs.getAbsoluteUrlFromClassPath(a);c.url=this._getUrlWithVersion(c.path);var d=this._loadFileSync(c.url);if(d.result){try{var e=getXmlDocument(d.data)}catch(f){return logger.error("XML error in class "+b+": "+f),c.isXmlError=!0,c.message=f,c}for(var g=e.getElementsByTagName("head")[0],e=e.getElementsByTagName("body")[0],h=[],m=[],q=c.path.substring(0,c.path.lastIndexOf("/")),l=0;l<g.childNodes.length;l++){var n=g.childNodes[l],p=n.nodeName.toLowerCase();if("script"==
p)try{this.addScriptNode(n,q,b,a)}catch(r){return a="Error processing <"+p+"> in "+b+" => "+r,logger.error(c.path+": "+a),c.message=a,c.stack=r.stack,c}else"link"==p?h.push(n):"style"==p&&m.push(n)}b=panjs._classes[a].Class;for(l=0;l<h.length;l++)this.addLinkNode(h[l],q);for(l=0;l<m.length;l++)logger.debug("Load style %1",a),this.addStyleNode(m[l]);if(defined(e))try{var s=getXml(e),t=b.prototype.parentClassName;defined(window[t].prototype.html)&&window[t].prototype.html.contains("\x3c!--CONTENT--\x3e")&&
(s=window[t].prototype.html.replace("\x3c!--CONTENT--\x3e",s));s=s.replace(/<body>/gi,"").replace(/<\/body>/gi,"");b.prototype.html=s;b.prototype.bodyNode=e}catch(u){return c.message=u.message,c}panjs.setSourceInComponents&&(b.prototype.source=d.data);b.prototype.classPathDir=this.getClassPathDir(a);b.prototype.dirPath=q;b.lastId=0;logger.groupEnd();logger.debug("END USESCOMPONENT %1",a);c.result=!0;c.message="ok";c.Class=b}else a="Error loading "+a+": "+d.status,logger.warn(a),c.mess=a}else c.result=
...
Tevent.HIDDEN="hidden";Tevent.WAITING="waiting";uses("panjs.core.events.Tevent");__CLASSPATH__="panjs.core.events.TeventDispatcher";
...
a={display:a.style.display,loaded:g,isCompo:f,visible:null,elem:a,excludeLogic:e,includeLogic:c,id:a.getAttribute("id"),includeIn:b,excludeFrom:d};this._statesElements.push(a)}},stateExists:function(a){var b=!1;if(null==a||""==a)return!1;for(var c=0;c<this._statesElements.length;c++){var d=this._statesElements[c];if(null!=d.includeIn&&0<=d.includeIn.indexOf(a)){b=!0;break}else if(null!=d.excludeFrom&&0<=d.excludeFrom.indexOf(a)){b=!0;break}}return b},_onSubComponentLoaded:function(a,b,c){c=a.container.attr("data-original-id");
panjs._setDOMId(a.container[0],c,a.classPath);this.bindEvents(b,a);c&&(this[c]=a);for(c=0;c<this._statesElements.length;c++){var d=this._statesElements[c];if(d.elem==b[0]){d.loaded=!0;d.elem=a.container[0];this.setStatesAttributes(b[0],d.elem);break}}},getChildren:function(){return this._children},findComponents:function(a){var b=$([]);this.container.getElements(0==arguments.length?"[data-compo]":'[data-compo*="'+a+'"]').each(function(a,d){d.isCompo()&&b.push(d[0].owner)});return b},setStatesAttributes:function(a,
b){a.getAttribute("data-include-in")&&b.setAttribute("data-include-in",a.getAttribute("data-include-in"));a.getAttribute("data-include-logic")&&b.setAttribute("data-include-logic",a.getAttribute("data-include-logic"));a.getAttribute("data-exclude-from")&&b.setAttribute("data-exclude-from",a.getAttribute("data-exclude-from"));a.getAttribute("data-exclude-logic")&&b.setAttribute("data-exclude-logic",a.getAttribute("data-exclude-logic"))},_setStateElementVisible:function(a,b,c){var d=!1;if(b.visible!=
a){var d=!0,e=$(b.elem);a?b.isCompo?b.loaded?b.elem.compo.show(c):e.load().show(c):defined(c,"showFn")?c.showFn(e,b):(e.show(c),"none"!=b.display&&e.css("display",b.display)):b.isCompo?b.loaded?b.elem.compo.hide(c):defined(c,"hideFn")?c.hideFn(e,b):e.hide(c):defined(c,"hideFn")?c.hideFn(e,b):e.hide(c);b.visible=a}return d},hasState:function(a){var b=!1;if("string"==typeof a)for(var c=0;c<this.currentState.length;c++){if(this.currentState[c]==a){b=!0;break}}else logger.error("hasState error: state is undefined");
...
d[this.currentState[g]]=1;this.currentState=[];for(var h=0;h<this._statesElements.length;h++){var m=this._statesElements[h],q=!0,l=!0;if(null!=m.includeIn)for(l=null,q=!0,g=0;g<m.includeIn.length;g++){var n=m.includeIn[g],n="undefined"!=typeof c[n];if("OR"==m.includeLogic){if(q=n)break}else q=q&&n}else if(null!=m.excludeFrom)for(l=!1,q=null,g=0;g<m.excludeFrom.length;g++)if(n=m.excludeFrom[g],n="undefined"!=typeof c[n],"OR"==m.excludeLogic){if(l=n)break}else l=l&&n;if(null!=q||null!=l)e=this._setStateElementVisible(!0==
q||!1==l,m,b)||e}this.currentState=[];for(var p in c)this.currentState.push(p),this.container.addClass(p);if(e){logger.isLevelEnabled(Tlogger.TRACE)&&logger.trace(this.id+".currentState = "+JSON.stringify(this.currentState));g=[];for(p in c)"undefined"==typeof d[p]&&g.push(p);h=[];for(p in d)"undefined"==typeof c[p]&&h.push(p);this.dispatchEvent(new Tevent(Tevent.STATE_CHANGED,{oldStates:f,newStates:this.currentState,removedStates:h,addedStates:g}))}return e},_populateElements:function(a,b,c){for(var d=
0;d<a.childNodes.length;d++){var e=a.childNodes[d];if(1==e.nodeType){var f=e.getAttribute("id"),g=e.getAttribute("data-compo"),h=$(e);if(null==g)b&&null!=f&&(defined(this[f])&&logger.error('1-Duplication du id "'+f+'" sur objet '+this.className),this[panjs.getCamelCase(f)]=h,panjs._setDOMId(e,f,g),c.push(f)),this.bindEvents(h),this.processStates(e),this._populateElements(e,b,c);else if(g==this.classPath)logger.error("R\u00e9f\u00e9rence circulaire dans "+this.classPath);else if("false"!==e.getAttribute("data-autoload")){var m=
this.createComponent(g,{elem:e,parent:this});m.parent=this;b&&null!=f&&(defined(this[f])&&logger.error('2-Duplication du id "'+f+'" sur objet '+this.className),this[f]=m,panjs._setDOMId(m.container[0],f,g),c.push(f));this.bindEvents(h,m);h.replaceWith(m.container);m.container.attr("data-loaded","true");e.setAttribute("data-loaded","true");m.container[0].loaded=!0;m.container[0].compo=m;this.setStatesAttributes(e,m.container[0]);this.processStates(m.container[0]);m._triggerOnAdded()}else panjs._setDOMId(e,
f,g),h[0].compo=null,h[0].owner=this,this[f]=h,this.processStates(e)}else 8==e.nodeType&&(f="",f=e.textContent,f=f.toUpperCase(),"CONTENT"==f&&(f=document.createElement("div"),f.setAttribute("data-class","content"),a.replaceChild(f,e),this.content=$(f)))}return c}});TdisplayObjectContainer._listeIdElements=[];__CLASSPATH__="panjs.core.display.Telement";
