...
defineClass("TsimpleGrid", "panjs.core.display.Telement", { 
	...
	{	
		...
		for (var i=start; i<= end; i++)
		{
			if (added < count){
				...
				if (this._addLine(d, docFragment))
					added ++;
			}
		}
	...
	{		
		...
		{
			var d = items[i];
			if (this.linesCount<this.sectionRecordsCount)
	...
	_setTr: function( tr, obj , trIsEmpty)
	{
		...
		if (typeof detailTr != "undefined")
		{
			...
			{
				//functionRenderer
				var content = this.detailRowRenderer(obj);
	...
	showDetailRow: function(item)
	{
		...
		if (detailTr == null)
		{
			...
			{
				//fonction
				var content = this.detailRowRenderer(item);
				if (typeof content == "object"){
					detailTd = $(content);
				}
				else{
...
<div id="tableContainer" style="height: 100%" >
    ...
    <!--CONTENT-->
     ...
     <h2 id="scrollWait" class="scrollWait">Chargement en cours...</h2>
</body>
</html> 