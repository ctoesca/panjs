...
defineClass("TlistView", "panjs.core.display.Telement", { 
	...
	getAllItemsRenderers: function(){
		var r = [];
		this.container.getElements('['+this._itemIdAttrName+']').each(function(indx, el){
			if (el.isCompo())
				r.push(el.getCompo());
		});
		return r;
	...
	{	
		var el = this.container.getElements('li['+this._itemIdAttrName+']'); 
	...
	createLI: function(item, visible){
		...
		{	
			...
			if (typeof html == "string"){
				...
				LI.innerHTML = this.functionRenderer(item);	
			}else{
				LI = html;
			}
