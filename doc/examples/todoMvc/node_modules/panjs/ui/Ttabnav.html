...
defineClass("Ttabnav", "panjs.core.display.Telement", { 
	...
	getTabByIndex: function(indx){
		var r = null;
		if (typeof this.tabs[indx] != "undefined")
			r = this.tabs[indx];
		return r;
	},
	setLabel: function(tab, value){
		...
		if (typeof tab == "number")
			var tab = this.getTabByIndex(tab);
	...
	_getSubComponentsFirstLevelNotLoaded: function( elem ){
		var r = [];
			...
			{			
				...
				if (dataType != null)
				{
					if (!loaded)
 					...
 					r.pushArray( this._getSubComponentsFirstLevelNotLoaded( el ) );
	...
	loadSubComponents: function(tab)
	{
		...
		var composToLoad = this._getSubComponentsFirstLevelNotLoaded(tab.elem[0])
		for (var i = 0; i < composToLoad.length; i++) {
			var compoElem = composToLoad[i];
			$(compoElem).load();
			/*var origId = compoElem.getAttribute("data-original-id");
