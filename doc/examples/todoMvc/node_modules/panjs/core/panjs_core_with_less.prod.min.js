...
setSource:function(a){if(this._source!=a){this._source=a;this._items=null!=this.filterFunction?this._source.filter(this.filterFunction):this._source.slice(0);if(null!=this.key)for(this._byId={},a=0;a<this._items.length;a++)this._byId[this._items[a][this.key]]=this._items[a];this.length=this._items.length;this.refresh()}},find:function(a){var b=[];if(a.filterFunction)for(var c=0;c<this._source.length;c++)a.filterFunction(this._source[c])&&b.push(this._source[c]);return b},swap:function(a,b){var c=
...
b)if(!0==c)e.push(g);else{e=g;break}}}}return e},getFilterFunction:function(){return this.filterFunction},setFilterFunction:function(a){this.filterFunction!=a&&(this.filterFunction=a)},dispatchUpdateEvent:function(a){this.dispatchEvent(new Tevent(Tevent.UPDATE,{item:a}));this.dispatchEvent(new Tevent(Tevent.CHANGE,{action:"UPDATE",item:a}))},refresh:function(){null!=this.filterFunction&&(this._items=this._source.filter(this.filterFunction));this.length=this._items.length;this.dispatchEvent(new Tevent(Tevent.REFRESH,
