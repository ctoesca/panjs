...
f=0;f<g.attributes.length;f++){var l=g.attributes.item(f),n=l.nodeName.toLowerCase(),m=l.value;n.startsWith("data-")&&(n=panjs.getFormatedArgName(n));"true"==m?m=!0:"false"==m&&(m=!1);b[n]=m}if("undefined"!=typeof panjs.root.components[a]&&"undefined"!=b.reuse&&!0===b.reuse){var q=panjs.root.components[a];q.reuse(b);return q}d=new window[c](b)}catch(h){g="Error instantiating "+c+": "+h,logger.error(g),uses("core.display.TerrorElement.html"),d=panjs.getAbsoluteUrlFromClassPath(a),l=panjs.loader.getUrlWithVersion(d),
d=new TerrorElement({message:h,path:d,url:l,className:c}),panjs.stack.push(g)}panjs.stackLevel--;0>=panjs.stackLevel&&(panjs.stack=[],panjs.stackLevel=0);d._onInitialized&&d._onInitialized();return panjs.root.components[a]=d};panjs.getFormatedIdName=function(a){if(-1==a.indexOf("-"))return a;var b="";a=a.split("-");for(var c=0;c<a.length;c++)b=0==c?a[c]:b+a[c].capitalizeFirstLetter();return b};panjs.getFormatedArgName=function(a){var b="";a=a.split("-");for(var c=1;c<a.length;c++)b=1==c?a[c]:b+a[c].capitalizeFirstLetter();
return b};panjs.loadScript=function(a,b){$LAB.script(a).wait(function(){"undefined"!=typeof b&&b(a)})};panjs.load=function(a){panjs._load(a)};panjs._load=function(a){0==arguments.length&&(a=$(document.body));var b=[];null!=a.attr("data-compo")?b.push(a):b=a.getElements("[data-compo]");for(var c=0;c<b.length;c++){var d=b[c],f=d.attr("data-compo"),g="false"!==d.attr("autoload"),l=d.attr("id");!0==g?(f=panjs.createComponent(f,{elem:d[0]}),d.replaceWith(f.container),f.container[0].owner=f):(uses("core.display.TproxyDisplayObject"),
f=new TproxyDisplayObject({sourceElement:d[0]}),f.parent=this,d[0].loaded=!1,d.hide(),d[0].setAttribute("id",panjs.root.id+"_"+l),d[0].originalId=l,d[0].owner=panjs.root);panjs.root[l]=f}logger.info("READY");"undefined"!=typeof onReady&&onReady()};panjs.getClassNameFromClassPath=function(a){var b=".";if(a.startsWith("http://")||a.startsWith("https://")||"/"==a[0])b="/";var c=a;a.endsWith(".html")&&(c=a.removeEnd(".html"));a=c.split(b);return c=a[a.length-1]};panjs.getNamespace=function(a){for(var b=
0;b<panjs.namespaces.length;b++){var c=panjs.namespaces[b];if(c.name==a)return c}return null};panjs.getAbsoluteUrlFromClassPath=function(a){var b=a.endsWith(".html"),c="";if(a.startsWith("http://")||"/"==a[0])c=b?a:a+".js";else for(var d=0;d<panjs.namespaces.length;d++){var f=panjs.namespaces[d];if(a.startsWith(f.name+".")){c=a.removeEnd(".html").replace(/\./g,"/");c=c.replace(f.name,f.path);c=b?c+".html":c+".js";break}}if(""==c)throw"Can't resolve "+a+": verify namespaces";return c};panjs.isAbsoluteUrl=
function(a){a=a.toLowerCase();return"/"==a[0]||a.toLowerCase().startsWith("http://")||a.toLowerCase().startsWith("https://")};panjs.transformUrl=function(a,b){"/"!=a[0]&&"http://"!=a.substring(0,7)&&"https://"!=a.substring(0,8)&&(a=b+"/"+a);return a}})();
...
n=[],m=[],q=c.substring(0,c.lastIndexOf("/")),h=0;h<l.childNodes.length;h++){var r=l.childNodes[h],s=r.nodeName.toLowerCase();if("script"==s)try{this.addScriptNode(r,q,a)}catch(u){return d="Error processing <"+s+"> in "+a+" => "+u,logger.error(c,d),{result:!1,message:d,className:a,classPath:b,url:e,path:c,stack:u.stack}}else"link"==s?n.push(r):"style"==s&&m.push(r)}if("undefined"==typeof window[a])return d=b+" class has not been loaded. ",panjs.lastDefinedClassName!=a&&(d+="\n"+panjs.getMessage(panjs.messages.CLASSNAME_MATCHS_FILENAME,
panjs.lastDefinedClassName,c)),logger.error(d),{result:!1,message:d,className:a,classPath:b,url:e,path:c,stack:null};for(h=0;h<n.length;h++)this.addLinkNode(n[h],q);for(h=0;h<m.length;h++)logger.debug("Load style ",b),this.addStyleNode(m[h]);if(defined(f))try{var p=getXml(f),t=window[a].prototype.parentClassName;defined(window[t].prototype.html)&&window[t].prototype.html.contains("\x3c!--CONTENT--\x3e")&&(p=window[t].prototype.html.replace("\x3c!--CONTENT--\x3e",p));p=p.replace(/<body>/gi,"").replace(/<\/body>/gi,
